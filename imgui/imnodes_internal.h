...
inline int ObjectPoolFindOrCreateIndex(ImObjectPool<ImNodeData>& nodes, const int node_id)
{
    ...
    if (node_idx == -1)
    {
        ...
        // Initial position: 150 px from top left corner
        ImNodeData& node = nodes.Pool[node_idx];
        node.Origin = ImVec2(150, 150) - GImNodes->CanvasOriginScreenSpace - editor.Panning;
    }
